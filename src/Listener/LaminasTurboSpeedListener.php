...
namespace Mecanik\LaminasTurboSpeed\Listener;
...
class LaminasTurboSpeedListener implements ListenerAggregateInterface
{
    // Zend\EventManager\ListenerAggregateTrait define the property ($listeners)
    use ListenerAggregateTrait;

    /**
     * LaminasTurboSpeed configuration!
     */
    private $config;
    ...
    public function onFinish(EventInterface $event)
    {
        ...
        if($this->config['engine_options']['js_minifier']['enabled'] === true) {
            if($this->config['engine_options']['js_minifier']['minify_inline_js']) {
                $content = preg_replace_callback('/(\\s*)<script(\\b[^>]*?>)([\\s\\S]*?)<\\/script>(\\s*)/i', [\Mecanik\LaminasTurboSpeed\Engine\LaminasTurboSpeedJSEngine::class, 'minify_js_callback'], $content);
            }
        }

        if($this->config['engine_options']['css_minifier']['enabled'] === true) {
            if($this->config['engine_options']['css_minifier']['minify_inline_styles']) {
                $content = preg_replace_callback('/\\s*<style(\\b[^>]*>)([\\s\\S]*?)<\\/style>\\s*/i', [\Mecanik\LaminasTurboSpeed\Engine\LaminasTurboSpeedCSSEngine::class, 'minify_css_callback'], $content);
            }    
        }

        if($this->config['engine_options']['html_minifier']['enabled'] === true) {
            if($this->config['engine_options']['html_minifier']['remove_comments']) {
                $content = preg_replace_callback('/<!--([\\s\\S]*?)-->/', [\Mecanik\LaminasTurboSpeed\Engine\LaminasTurboSpeedHTMLEngine::class, 'remove_comments_callback'], $content);
            }
        ...
        if($this->config['engine_options']['http2_push']['enabled'] === true) {
            ...
            $http_pusher = new \Mecanik\LaminasTurboSpeed\Engine\LaminasTurboSpeedHTTP2Engine($event->getApplication()->getResponse(), $content, $event->getApplication()->getRequest());
        ...
        if($this->config['show_credits'] === true) {
            $response->getHeaders()->addHeaderLine("X-Supercharged-By: Laminas Turbo Speed (https://github.com/Mecanik/LaminasTurboSpeed)");
        }
